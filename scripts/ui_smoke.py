...
with sync_playwright() as p:
    ...
    page.locator("nav").get_by_role("button", name="任务", exact=False).click()
    page.locator(".tasks-list").wait_for()
    assert page.locator(".task-row").count() >= 1
    page.screenshot(path=str(ARTIFACTS / "control-panel-tasks.png"), full_page=True)
    page.get_by_role("button", name="实例", exact=True).click()
    ...
    page.get_by_role("button", name="资源", exact=True).click()
    page.locator(".metrics-tool").wait_for()
    assert page.locator(".metrics-toolbar button").count() == 4
    page.screenshot(path=str(ARTIFACTS / "control-panel-metrics.png"), full_page=True)
    ...
    page.get_by_role("button", name="资源", exact=True).click()
    page.locator(".metrics-tool").wait_for()
    page.screenshot(path=str(ARTIFACTS / "control-panel-metrics-mobile.png"), full_page=True)
    page.get_by_title("打开导航").click()
    page.locator("nav").get_by_role("button", name="任务", exact=False).click()
    page.locator(".tasks-list").wait_for()
    page.wait_for_timeout(400)
    page.screenshot(path=str(ARTIFACTS / "control-panel-tasks-mobile.png"), full_page=True)
    page.get_by_title("打开导航").click()
    page.get_by_role("button", name="实例", exact=True).click()
    page.get_by_role("button", name="计划任务", exact=True).click()
